      ...
      self.class_eval do
        ...
        define_method("#{__attr_name}="){ |new_value|
          ...
          if instance_variable_get(var_name).nil?
            ...
            raise Dilithium::DomainObjectExceptions::ImmutableObjectError, "#{self.class.name} is immutable. The value of #{__attr_name} cannot be changed once it has been set" unless __attr_name == :active
          end
        }
      end
    end
